...
const ChoseColor = ({ navigation }) => {
  const [selectedColor, setSelectedColor] = useState(''); // Màu mặc định
  const [productImage, setProductImage] = useState(
    require('./img/vs_blue.png')
  ); // Ảnh sản phẩm mặc định

  const [text1, setText1] = useState(''); // Màu mặc định
  const [text2, setText2] = useState(''); // Màu mặc định
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.part2}>
        ...
        <View style={styles.colorButtons}>
          <TouchableOpacity
            style={[styles.colorButton, { backgroundColor: 'lightblue' }]}
            onPress={() =>
              changeProductColor(
                'Bạc',
...
const styles = StyleSheet.create({
  ...
  colorButtons: {
    alignItems:'center',
    flexDirection: 'column', // Đổi từ 'row' thành 'column'
    marginBottom: 20,
  },
  colorButton: {
    width: 50,
    ...
    borderWidth:1,
    borderColor:'white',
    marginBottom: 10, // Khoảng cách giữa các nút màu
  },
  doneButton: {
    width: 350,
    height: 44,
